 ...
 * $Revision: 1 $
 * $Date: 15/04/01 1:02p $ 
 * @brief    XN11E retarget source file
 *
 * @note
 * Copyright (C) 2014 Xinnova Technology Corp. LTD All rights reserved.
*****************************************************************************/ 
#include <stdio.h>
#include "XN_UART.h"
#if defined ( __CC_ARM   )
...
{
#ifdef ENABLE_UART0
        while(XN_UART0->STAT.bit.TXF);
        XN_UART0->DAT.bit.DATA= ch;
        if(ch == '\n'){
            while(XN_UART0->STAT.bit.TXF);
            XN_UART0->DAT.bit.DATA= '\r';
        }
#endif
}
...
#ifdef ENABLE_UART0
        while (1){
            if(XN_UART0->STAT.bit.RXNE)
            {
                return (XN_UART0->DAT.bit.DATA);
...
{
#ifdef ENABLE_UART0
    return (XN_UART0->STAT.bit.TXF) ? 1 : 0;
#else
    return(1);
#endif    
...
/*** (C) COPYRIGHT 2013 Xinnova Technology Corp. LTD***/
