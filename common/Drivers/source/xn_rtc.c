/*****************************************************************************
 *   xn_rtc.h:  Header file for Xinnova XN11E
 *
 ...
 *	 Note: This driver may be need to change depends on MCU version. Please contact 
 *   vendor for support.
******************************************************************************/
/* Includes ------------------------------------------------------------------*/
...
Input Parameters	*dt: datetime defined in XN_RTC.h
...
Input Parameters	seconds: seconds since 2000:01:01 00:00
									*dt: datetime defined in XN_RTC.h
Return Value	No
Condition	No
Function called	-
...
{
	#if OLD_VERSION
		XN_RTC->LR.all=RTC_DateTimetoSecond(dt);
	#else
		XN_RTC->LR.all=RTC_DateTimetoSecond(dt)<<1;
	#endif		
...
{
	#if OLD_VERSION
		RTC_SecondtoDateTime(XN_RTC->DR.all, dt);
	#else
		RTC_SecondtoDateTime(XN_RTC->DR.all>>1, dt);
	#endif		
...
void RTC_SetAlarmDateTime (DateTime_TypeDef *dt)
{
	...
	#if OLD_VERSION
		XN_RTC->MR.all=RTC_DateTimetoSecond(dt);
	#else
		XN_RTC->MR.all=RTC_DateTimetoSecond(dt)<<1;
	#endif	
	
	//enable rtc interrupt
	XN_RTC->ICSC.bit.RTCIC=1;
	return;		
}
/*****************************************************************************
Function Name	RTC_SetAlarmDateTime
Function Definition	void RTC_SetAlarmInTickLater(uint32_t ticks)
Function Description	Set alarm in ticks 
Input Parameters	ticks: half second ticks
Return Value	No
Condition	No
Function called	-
...
{
	//set alarm match register
	XN_RTC->MR.all=XN_RTC->DR.all+ticks;
	//enable rtc interrupt
	XN_RTC->ICSC.bit.RTCIC=1;
	return;
}
