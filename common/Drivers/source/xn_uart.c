/*****************************************************************************
 *   xn_uart.h:  Header file for Xinnova XN11E
 *
...
Function Definition	void UART_Open(XN_UART_TypeDef *uart, uint32_t baudrate, uint8_t parityoption, uint8_t rxinttriggerlevel);
Function Description	Initial UART 
Input Parameters	*uart : pointer to UART device, XN_UART0/XN_UART1
...
*****************************************************************************/
void UART_Open(XN_UART_TypeDef *uart, uint32_t baudrate, uint8_t parityoption, uint8_t rxinttriggerlevel)
{
	...
	if (uart==XN_UART0)
	{
		...
		//Enable UART clock
		XN_SYSCON->SYSAHBCLKCTRL.bit.UART0CLK=1;	//enable UART0 PCLK
		XN_SYSCON->UART0CLKDIV.bit.DIV = 0x1;     /* divided by 1 */
		//reset UART
		XN_SYSCON->PRESETCTRL.bit.UART0_RST_N=0;
		XN_SYSCON->PRESETCTRL.bit.UART0_RST_N=1;
	}else if (uart==XN_UART1)
	{
		//disable UART0 IRQ during setup
		NVIC_DisableIRQ(UART1_IRQn);
		/* Enable UART clock */
		XN_SYSCON->SYSAHBCLKCTRL.bit.UART1CLK=1;	//enable UART1 PCLK
		XN_SYSCON->UART1CLKDIV.bit.DIV = 0x1;     /* divided by 1 */
		//reset UART
		XN_SYSCON->PRESETCTRL.bit.UART1_RST_N=0;
		XN_SYSCON->PRESETCTRL.bit.UART1_RST_N=1;
	}else return ;
  ...
  uart->CTRL.bit.TXEN=1; 
	...
	//insert delay
	delays=uart->BAUDDIV.bit.BAUDDIV;
	while(delays--);
	//clr FIFO
...
Function Definition	void UART_Close(XN_UART_TypeDef *uart);
...
Last Chang Date: 2015/09/12			By:YL
*****************************************************************************/
void UART_Close(XN_UART_TypeDef *uart)
{
	if (uart==XN_UART0)
	{
		//disable UART0 IRQ during setup
		NVIC_DisableIRQ(UART0_IRQn);
		//Disable UART clock
		XN_SYSCON->SYSAHBCLKCTRL.bit.UART0CLK=0;	
	}else if (uart==XN_UART1)
	{
		//disable UART0 IRQ during setup
		NVIC_DisableIRQ(UART1_IRQn);
		//Disable UART clock
		XN_SYSCON->SYSAHBCLKCTRL.bit.UART1CLK=0;
	}else return ;	
	//Disable UART int
...
Function Definition	Uint8_t UART_ByteRead(XN_UART_TypeDef *uart, uint8_t *data);
...
Last Chang Date: 2015/09/12			By:YL
*****************************************************************************/
uint8_t UART_ByteRead(XN_UART_TypeDef *uart, uint8_t *data)
{
...
Function Definition	Uint8_t UART_Read(XN_UART_TypeDef *uart, uint8_t * rxbuf, uint8_t *readbytes);
...
Last Chang Date: 2015/09/12			By:YL
*****************************************************************************/
void UART_Read(XN_UART_TypeDef *uart, uint8_t * rxbuf, uint8_t *readbytes)
{
	uint8_t temp=0;
	//get all data
...
Function Definition	Uint8_t UART_ByteWrite(XN_UART_TypeDef *uart, uint8_t data);
...
Last Chang Date: 2015/09/12			By:YL
*****************************************************************************/
uint8_t UART_ByteWrite(XN_UART_TypeDef *uart, uint8_t data)
{
...
Function Definition	Uint8_t UART_Write(XN_UART_TypeDef *uart, uint8_t * txbuf, uint8_t writebytes);
...
Last Chang Date: 2015/09/12			By:YL
*****************************************************************************/
uint8_t  UART_Write(XN_UART_TypeDef *uart, uint8_t * txbuf, uint8_t writebytes)
{
...
Function Definition	void UART_Send(XN_UART_TypeDef *uart, uint8_t * txbuf, uint32_t sendbytes);
...
Last Chang Date: 2015/09/12			By:YL
*****************************************************************************/
void UART_Send(XN_UART_TypeDef *uart, uint8_t * txbuf, uint32_t sendbytes)
{
	while (sendbytes--)
	{
...
Function Definition	void UART_PutString (XN_UART_TypeDef *uart, uint8_t * str);
...
Last Chang Date: 2015/09/12			By:YL
*****************************************************************************/
void UART_PutString (XN_UART_TypeDef *uart, uint8_t * str)
{
	while (!(* str=='\0'))
	{
		while (uart->STAT.bit.TXF);
...
Function Definition	void UART_PutHex (XN_UART_TypeDef *uart, uint32_t data);
...
Last Chang Date: 2015/09/12			By:YL
*****************************************************************************/
void UART_PutHex (XN_UART_TypeDef *uart, uint32_t data)
{
	uint8_t hex;
	uint8_t len=7;
	do
...
Function Definition	void UART_EnableInt(XN_UART_TypeDef *uart, uint32_t intcon);
...
Last Chang Date: 2015/09/12			By:YL
*****************************************************************************/
void UART_EnableInt(XN_UART_TypeDef *uart, uint32_t intcon)
{
...
Function Definition	void UART_DisableInt(XN_UART_TypeDef *uart);
...
Last Chang Date: 2015/09/12			By:YL
*****************************************************************************/
void UART_DisableInt(XN_UART_TypeDef *uart)
{
...
Function Definition	void UART_ClearIntFlag(XN_UART_TypeDef *uart);
...
Last Chang Date: 2015/09/12			By:YL
*****************************************************************************/
void UART_ClearIntFlag(XN_UART_TypeDef *uart)
{
...
Function Definition	void UART_GetStatus (XN_UART_TypeDef *uart);
...
Last Chang Date: 2015/09/12			By:YL
*****************************************************************************/
uint8_t UART_GetStatus(XN_UART_TypeDef*uart)
{
	return (uint8_t) uart->STAT.all;
}
/*****************************************************************************
Function Name	UART_ClearFIFO
Function Definition	void UART_ClearFIFO(XN_UART_TypeDef *uart,);
...
Last Chang Date: 2015/09/12			By:YL
*****************************************************************************/
void UART_ClearFIFO(XN_UART_TypeDef *uart)
{
	uart->FIFOCLR.all=0xFF;
	return ;
}
